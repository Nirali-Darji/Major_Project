...
function DisplayOption() {
  ...
  return (
    <div>
      <div className="flex gap-2 items-center bg-gray-100 p-2 fixed top-30 right-75 rounded-lg z-10">
        {buttons.map((btn) => (
          <button
            key={btn.id}
            onClick={() => setSelected(btn.id)}
            className={`p-1 rounded-lg transition-all duration-200 hover:bg-gray-300 ${
              selected === btn.id ? "border-2 border-blue-500" : "border-2 border-transparent"
            }`}
          >
            {btn.icon}
          </button>
